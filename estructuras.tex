...
\begin{definition}\label{eq:normalizador} Si $X$ es un subconjunto no vacío de un grupo $G$, se llama \textbf{normalizador} de $X$ en $G$ a $$N_{G}(X) = \lbrace a \in G : X^{a} = X\rbrace,$$ que además es un subgrupo de $G$.
\end{definition}
...
\item Si $x \in G$, $x^{-1}x = 1 \in H$ luego $x\sim_{H}x$.\vspace{0.2cm}\\
\item Si $x\sim_{H}y$ entonces $x^{-1}y \in H$, luego $(x^{-1}y)^{-1} \in H,$ y esto es $y^{-1}x \in H$ así que $y\sim_{H}x$.\vspace{0.2cm}\\
...
Sea ahora $\left[ x \right]_{\sim^{H}}$ la clase de equivalencia del elemento $x \in G$ definida por la relación $\sim_{H}$. Entonces \begin{center}$\left[ x \right]_{\sim_{H}} = \lbrace a \in G : x\sim_{H}a \rbrace = \lbrace a \in G : x^{-1}a = h \in H \rbrace = \lbrace a \in G: a = xh, \hspace{0.1cm} h \in H \rbrace = xH.$\end{center}

Análogamente podemos hacer con $\sim^{H}$, \begin{center}$\left[ x \right]_{\sim^{H}} = \lbrace a \in G : x\sim^{H}a \rbrace = \lbrace a \in G: ax^{-1} = h \in H \rbrace = \lbrace a \in G : a = hx, \hspace{0.1cm} h \in H \rbrace = Hx.$\end{center} Notar que en este último caso tomamos los $h$ de la forma $ax^{-1}$ cuando la relación $\sim^{H}$ en realidad vendría a decir que $h$ sería de la forma $xa^{-1}$, simplemente tomamos el inverso (que también está en $H$) ya que la relación es de equivalencia es igual da hacer $x \sim^{H}a$ que $a\sim^{H}x$.
...
\item Si $a \in H$. En tal caso, $a^{-1}1 = a \in H$ y así $a\sim_{H}1$, luego $aH = 1H = H.$ Y como $a1^{-1} = a \in H$, entonces $a\sim^{H}1$ y así, $Ha = H1 = H.$ Por lo tanto, $$aH = Ha.$$
...
\begin{proposition}Supongamos que $N \unlhd G$. El conjunto $G/N$ de las clases a izquierda módulo $N$ es un grupo con la operación de $G$ 
...
\end{array}
$ es claramente un isomorfismo. Ya sabemos que si $G_{1}\cong G_{2}$ entonces $G_{2} \cong G_{1}$, y además si tenemos un tercer grupo $G_{3}$ y $G_{1} \cong G_{2}$, $G_{2} \cong G_{3}$, entonces $G_{1} \cong G_{3}$ ya que la composición de isomorfismos también es isomorfismo. Por lo tanto, si consideramos el conjunto de todos los grupos, la relación binaria $\cong$ es de equivalencia.
...
donde $\pi$ e $i$ son los homomorfismos proyección e inclusión presentados anteriormente. 
...
Es claro que $H \unlhd N_G(H)$ y que $H \unlhd G$ si y sólo si $N_G(H) =G$. Por el \textit{Teorema de la órbita estabilizadora} tenemos que el número de subgrupos distintos de la forma $H^g$, con $g \in G$, es $[G:N_G(H)]$. Es decir, que el número de conjugados distintos de un subgrupo $H$ de $G$ viene dado por $[G:N_G(H)]$.
...
Notar que todas las transposiciones tienen orden $2$, y que por tanto si $t$ es una transposición entonces $t=t^{-1}$.
